...
// Supprimer un commentaire en utilisant la méthode delete
 ...
 public void shareFacebook(Blog blog) {
    ...
    public List<Commentaire> getCommentsByBlogId(int blogId) {
        ...
        String query = "SELECT commentaire.id, commentaire.contenue, admin.id AS admin_id, admin.cin AS admin_cin, admin.nom AS admin_nom, admin.prenom AS admin_prenom, admin.genre AS admin_genre FROM commentaire JOIN admin ON commentaire.idadmin_id = admin.id WHERE commentaire.idblog_id = ?";
        try {
            ...
            while (resultSet.next()) {
                ...
                admin.setGenre(resultSet.getString("admin_genre"));
